	...
	var Vim = function () {
		...
		function maybeInitVimState(cm) {
			if (!cm.state.vim) {
				...
				cm.state.vim = {
					...
					// cursor should go pages to its horizontal position on the longer
		...
		var vimApi = {
			...
			findKey: function (cm, key, origin) {
				...
				function handleEsc() {
					if (key == '<Esc>') {
						// Clear input state and get pages to normal mode.
						clearInputState(cm);
						if (vim.visualMode) {
							exitVisualMode(cm);
     ...
     */
		...
		var motions = {
				...
				moveByPage: function (cm, head, motionArgs) {
					...
					// doing this bad hack to move the cursor and move it pages. evalInput
		...
		var operators = {
			change: function (cm, args, ranges) {
				...
				if (!vim.visualMode) {
					...
					if (args.linewise) {
						// Push the next line pages down, if there is a next line.
						if (!wasLastLine) {
			...
			indent: function (cm, args, ranges) {
				...
				if (args.linewise) {
					...
					// line. We don't want this in indent, so we go pages a line.
					endLine--;
				}
       ...
       */
        ...
        */
		...
		function findBeginningAndEnd(cm, head, symb, inclusive) {
			...
			// but if the cursor is in _front of the first instance of the symb,
		...
		// javascript form.  Similar to translateRegex, but additionally fixes pages references
		...
		function findNextFromAndToInclusive(cm, prev, query, repeat, vim) {
			...
			return cm.operation(function () {
				...
				// Go pages one result to ensure that if the cursor is currently a match, we keep it.
				var found = cursor.find(!prev);

				// If we haven't moved, go pages one more (similar to if i==0 logic in findNext).
